...
type MainTab = "asistente_ia" | "ordenes" | "recepciones" | "facturas_p2p" | "devoluciones" | "matching" | "proveedores" | "requisiciones" | "cotizaciones" | "presupuestos" | "reportes"
...
export default function PurchasesPage() {
  ...
  const handleConfirmPO = async (id: string) => {
    try {
      ...
      fetchAll()
      if (selectedPO?.id === id) {
        setSelectedPO(prev => prev ? { ...prev, estado: "confirmado" } : null)
      }
    } catch (e: any) {
  ...
  const handleSendPO = async (id: string) => {
    try {
      ...
      fetchAll()
      if (selectedPO?.id === id) {
        setSelectedPO(prev => prev ? { ...prev, estado: "enviada" } : null)
      }
    } catch (e: any) {
  ...
  const handleCancelPO = async (id: string) => {
    ...
    try {
      ...
      fetchAll()
      if (selectedPO?.id === id) {
        setSelectedPO(prev => prev ? { ...prev, estado: "cancelado" } : null)
      }
    } catch (e: any) {
  ...
  const handleOpenGenerateModal = () => {
    const itemsToOrder = (replenishmentData?.items || []).filter(
      it => selectedItemsIA[it.product_id] && (editedQuantities[it.product_id] || 0) > 0
    )
  ...
  const montoEnTransito = useMemo(() => {
    return ordenesEnTransito.reduce((acc, o) => acc + Number(o.total || 0), 0)
  }, [ordenesEnTransito])
  ...
  const totalOrdenIASugerida = useMemo(() => {
    ...
    return replenishmentData.items
      .filter(it => selectedItemsIA[it.product_id])
      .reduce((acc, it) => {
  ...
  const displayedReplenishmentItems = useMemo(() => {
    ...
    return replenishmentData.items.filter(it => {
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left text-xs min-w-[1100px]">
                  <thead className="bg-slate-100/80 dark:bg-slate-900/50 text-gray-500 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200 dark:border-slate-700/60 sticky top-0">
                    <tr>
                      <th className="p-3 w-10 text-center">
                        <input
                          type="checkbox"
                          checked={displayedReplenishmentItems.length > 0 && displayedReplenishmentItems.every(it => selectedItemsIA[it.product_id])}
                          onChange={(e) => {
                            const checked = e.target.checked
                            const newSel = { ...selectedItemsIA }
                            displayedReplenishmentItems.forEach(it => { newSel[it.product_id] = checked })
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {displayedReplenishmentItems.map((it) => {
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700/60 space-y-2">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-bold font-mono text-gray-800 dark:text-gray-200">
                    {(replenishmentData?.items || []).filter(it => selectedItemsIA[it.product_id] && (editedQuantities[it.product_id] || 0) > 0).length} productos
                  </span>
                </div>
                <div className="flex justify-between text-xs">
