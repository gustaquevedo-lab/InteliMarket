...
import {
  ...
  Package, Users, MapPin, Calendar, DollarSign, Zap, ThumbsUp, ThumbsDown, Clock,
  FileSpreadsheet, RefreshCcw, BrainCircuit, ArrowUpDown, LineChart, PieChart,
} from "lucide-react"
    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Predicción ML, detección de anomalías, sugerencias de compra, precisión</p>
        </div>
      </div>
...
function ComprasTab() {
  ...
  const load = () => {
    setLoading(true)
    api.demandForecast?.listPurchaseSuggestions(COMPANY_ID).then(setSuggestions).finally(() => setLoading(false))
  }
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {suggestions.map((s: any) => (
              <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 text-right">{s.current_stock != null ? Intl.NumberFormat().format(s.current_stock) : "—"}</td>
            ...
            {suggestions.length === 0 && (
              <tr><td colSpan={8} className="text-center py-8 text-gray-400">Sin sugerencias de compra. Generá primero un forecast.</td></tr>
            )}
          </tbody>
        </table>
      </div>
