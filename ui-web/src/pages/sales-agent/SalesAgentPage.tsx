...
import { TrendingUp, Sparkles, CheckCircle2, XCircle, Loader2, AlertTriangle, RefreshCw } from "lucide-react"
import { api, type SalesRecommendation, type SalesAgentRun } from "../../api"
...
const TIPO_LABEL: Record<string, string> = {
  ...
  concentracion_cliente: "Concentración de riesgo",
  estacionalidad: "Estacionalidad",
  otro: "Otro",
}

export default function SalesAgentPage() {
  const [recs, setRecs] = useState<SalesRecommendation[]>([])
  const [loading, setLoading] = useState(true)
  const [running, setRunning] = useState(false)
  const [deciding, setDeciding] = useState<string | null>(null)
  const [filterStatus, setFilterStatus] = useState<string>("pending")
  const [lastRun, setLastRun] = useState<SalesAgentRun | null>(null)
  ...
  const fetchRecs = async () => {
    setLoading(true)
    try {
      const data = await api.salesAgent.recommendations(filterStatus === "todos" ? undefined : filterStatus)
      setRecs(data)
    } catch {
      setRecs([])
  ...
  const runDiagnosis = async () => {
    setRunning(true)
    try {
      const run = await api.salesAgent.run()
      setLastRun(run)
      if (run.status === "error") {
        toast.error(run.error_message || "El Gerente de Ventas IA no pudo completar el diagnóstico")
      } else {
  ...
  const decide = async (id: string, approve: boolean) => {
    ...
    setDeciding(id)
    try {
      await (approve ? api.salesAgent.approve(id, user.id) : api.salesAgent.reject(id, user.id))
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Gerente de Ventas IA</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Diagnóstico, escenarios y recomendaciones — vos aprobás, nunca actúa solo</p>
          </div>
        </div>
            ...
            <div key={r.id} className="p-4 flex items-start gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                  {r.entidad_relacionada && <span>🏷️ {r.entidad_relacionada}</span>}
