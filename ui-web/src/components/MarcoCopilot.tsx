...
import {
  ...
  Bot, User, ChevronDown, ChevronUp, Loader2, MessageSquare, Play, Pause, Check, Square
} from "lucide-react"
...
function FormattedExecutiveMessage({ content, isUser }: { content: string; isUser?: boolean }) {
  if (isUser || !content) {
    return <p className="text-xs whitespace-pre-wrap font-medium">{content}</p>
  }
    ...
    <div className="space-y-2.5 text-xs leading-relaxed">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith("###") || trimmed.startsWith("##")) {
          const headerText = cleanText(trimmed.replace(/^#+\s*/, ""))
          return (
            <h4 key={idx} className="font-bold text-gray-900 dark:text-white text-xs mt-3 mb-1.5 flex items-center gap-1.5 border-b border-gray-100 dark:border-gray-700/60 pb-1">
        ...
        // Numbered step / action list: 1. Combo...
        const numMatch = trimmed.match(/^(\d+)\.\s*(.*)/)
        if (numMatch) {
          const num = numMatch[1]
          const rest = numMatch[2]
          return (
            <div key={idx} className="flex items-start gap-2 p-2.5 bg-slate-50 dark:bg-gray-800/80 rounded-xl border border-gray-100 dark:border-gray-700/60 shadow-2xs">
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-[95vw] sm:w-[460px] h-[640px] max-h-[85vh] bg-white dark:bg-gray-850 rounded-3xl shadow-2xl border border-gray-200/80 dark:border-gray-700/80 flex flex-col overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-bottom-5 duration-300">
          ...
          {/* Active Audio Bar with Stop Button */}
          {playingAudioId !== null && (
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2 text-white flex items-center justify-between text-xs font-semibold shadow-inner animate-in fade-in">
              <div className="flex items-center gap-2">
                <Volume2 className="w-4 h-4 animate-bounce text-emerald-200" />
                <span>Marco está hablando...</span>
              </div>
              <button
                onClick={stopAudio}
                className="flex items-center gap-1.5 px-2.5 py-1 bg-white/20 hover:bg-white/30 rounded-xl text-[11px] font-bold transition active:scale-95 text-white shadow-sm"
                title="Detener voz inmediatamente"
              >
